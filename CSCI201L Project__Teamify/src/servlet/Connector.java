	...
	public List<User> getUserByTag(List<String> tags) {
		...
		try {
			...
			while(it.hasNext()) {
				...
				rs = ps.executeQuery();
				target.add(new User(rs.getString("Username"), rs.getString("Description")));
			}
	...
	public List<Project> getProjectByTag(List<String> tags) {
		...
		try {
			...
			while(it.hasNext()) {
				...
				rs = ps.executeQuery();
				while(rs.next()) {
					target.add(new Project(rs.getString("ProjectName"), rs.getString("Description"), rs.getString("Image")));
				}
			}
